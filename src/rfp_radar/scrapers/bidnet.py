"""BidNet portal scraper for RFP Radar.

This module provides a scraper for BidNet Direct, a procurement network
that serves government agencies and provides access to public sector
bid opportunities.

BidNet provides a centralized platform for government procurement
opportunities at federal, state, and local levels.
"""
...
@register_scraper("bidnet")
class BidNetScraper(BaseScraper):
    """Scraper for BidNet Direct procurement portal.

    BidNet provides access to government procurement opportunities
    across federal, state, and local agencies. This scraper fetches
    bid opportunities and normalizes them into the RFP format.

    Attributes:
        source: RFPSource.BIDNET
        base_url: BidNet API base URL (placeholder - requires authentication)

    Example:
        with BidNetScraper(max_age_days=3) as scraper:
            result = scraper.scrape()
            for rfp in result.rfps:
                print(f"Found: {rfp.title}")
    """

    source: RFPSource = RFPSource.BIDNET
    base_url: str = "https://api.bidnet.com/v1"

    # BidNet-specific configuration
    opportunities_endpoint: str = "/solicitations"
    default_page_size: int = 50
    max_pages: int = 10

    def _configure_session(self) -> None:
        """Configure the requests session with BidNet-specific headers."""
        super()._configure_session()
        # BidNet API may require additional headers
    ...
    def fetch_listings(
        ...
        **kwargs,
        ...
        """Fetch raw RFP listings from BidNet API.

        Fetches solicitations from BidNet, handling pagination automatically.
        ...
        Args:
            ...
            agency_type: Optional agency type filter (e.g., "federal", "state", "local")
            **kwargs: Additional query parameters

        Returns:
            List of raw solicitation dictionaries from the API
        ...
        self.logger.info(
            "Completed fetching BidNet solicitations",
    ...
    def _build_query_params(
        ...
        **kwargs,
        ...
        """Build query parameters for the BidNet API.
        ...
        Args:
            ...
            agency_type: Optional agency type filter
            **kwargs: Additional query parameters
        ...
        params = {
            "page": page,
            "limit": page_size,
            "sort": "-publish_date",  # Sort by most recent first
            "status": "active",  # Only active solicitations
        }
    ...
    def _extract_listings(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if isinstance(data, list):
            ...
            for key in ["solicitations", "bids", "opportunities", "results", "data", "items"]:
                if key in data and isinstance(data[key], list):
                    return data[key]
            # If the dict looks like a single solicitation
            if "id" in data or "title" in data or "solicitation_number" in data:
                return [data]
        return []

    def parse_listing(self, raw_data: Dict[str, Any]) -> Optional[RFP]:
        """Parse a raw BidNet listing into an RFP object.

        Normalizes the BidNet-specific data format into the
        standard RFP model.

        Args:
            raw_data: Raw solicitation data from BidNet API
        ...
        try:
            ...
            title = self._extract_field(
                raw_data,
                ["title", "name", "solicitation_title", "bid_title", "subject"],
            )
            if not title:
                self.logger.debug(
            ...
            posted_date = self._parse_date(
                self._extract_field(
                    raw_data,
                    ["posted_date", "publish_date", "created_at", "issue_date", "open_date"],
                )
            )
            due_date = self._parse_date(
                self._extract_field(
                    raw_data,
                    ["due_date", "close_date", "deadline", "response_deadline",
                     "submission_deadline", "bid_due_date"],
                )
            )
            ...
            rfp = RFP(
                ...
                description=self._extract_field(
                    raw_data,
                    ["description", "summary", "synopsis", "details", "scope", "overview"],
                    default="",
                ),
                agency=self._extract_field(
                    raw_data,
                    ["agency", "organization", "department", "issuer", "buyer",
                     "government_entity", "purchasing_agency"],
                    default="",
                ),
                source=self.source,
                source_url=self._extract_field(
                    raw_data,
                    ["url", "source_url", "link", "bid_url", "portal_url", "solicitation_url"],
                    default="",
                ),
                ...
                set_aside=self._extract_field(
                    raw_data,
                    ["set_aside", "set_aside_type", "preference", "small_business"],
                    default="",
                ),
            ...
            self.logger.warning(
                "Failed to parse BidNet listing",
                extra={
                    "error": str(e),
    ...
    def _generate_id(self, raw_data: Dict[str, Any]) -> str:
        ...
        for key in ["id", "_id", "bid_id", "solicitation_id", "solicitation_number",
                    "bid_number", "reference_number"]:
            if key in raw_data and raw_data[key]:
                return f"bidnet-{raw_data[key]}"
    ...
    def _parse_date(self, date_value: Any) -> Optional[datetime]:
        ...
        if isinstance(date_value, str):
            date_formats = [
                ...
                "%m/%d/%Y",
                "%m/%d/%Y %H:%M:%S",
                "%m/%d/%Y %I:%M %p",  # e.g., "01/15/2024 02:30 PM"
                "%d-%m-%Y",
    ...
    def _parse_location(
        ...
        # BidNet primarily serves US government agencies
